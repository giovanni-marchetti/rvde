...
EuclideanKernelGPU::EuclideanKernelGPU(const dmatrix &points, const ptr<Bounds> &bounds) : EuclideanKernel(points,
                                                                                                           bounds) {
    // initialize gpu
    if (!context) {
    ...
    intersect_ray_indexed_boundary_kernel = init_kernel("intersect_ray_indexed_boundary",
                                               "-D INTERSECT_RAY_INDEXED_BOUNDARY_GS0=" + std::to_string(intersect_ray_indexed_gs[0])
                                               + " -D INTERSECT_RAY_INDEXED_BOUNDARY_GS1=" + std::to_string(intersect_ray_indexed_gs[1]));
...
void EuclideanKernelGPU::reset_reference_points_gpu(const dmatrix &ref_mat, const vec<vec<int>> &duals) {
