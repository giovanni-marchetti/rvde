...
void set_silent(bool flag) {
    static std::streambuf* true_buf = std::cout.rdbuf();
    static std::ofstream null_file(NULL_DEVICE);
    static FILE original_stdout = *stdout;

    if (flag) {
        std::cout.rdbuf(null_file.rdbuf());
        *stdout = *fopen(NULL_DEVICE,"w");
    } else {
        std::cout.rdbuf (true_buf);
